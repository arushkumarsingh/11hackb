...
def main(video_path: str, output_path: str | None = None, reencode: bool = True):
    ...
    frames_dir = Path(__file__).parent / f"{base_name}_frames"
    frames_dir.mkdir(exist_ok=True)
    frames_dir_str = str(frames_dir)
    
    # Step 1: Extract frames using ffmpeg
    logger.info("📸 Step 1: Extracting frames from video...")
    extract_frames_every_second(video_path, frames_dir_str)
    
    # Get list of frame files, sorted by name (which corresponds to time)
    frame_files = sorted([f for f in os.listdir(frames_dir_str) if f.startswith("frame_") and f.endswith(".png")])
    ...
    for i, frame_file in enumerate(frame_files):
        frame_path = os.path.join(frames_dir_str, frame_file)
